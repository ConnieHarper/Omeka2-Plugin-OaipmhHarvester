...
abstract class OaipmhHarvester_Harvest_Abstract
{
	...
	// Get the base URL.
        ...
        if ($this->_harvest->identifier) {
                $response = $this->_harvest->getRecord();
        } else {
    ...
    private function _harvestLoop($record)
    {
		...
		// skip over records that don't fit the search criteria if searching unless you already have an identifier.
        if ($this->_harvest->search !== '' && !$this->_harvest->identifier){
    ...
    final protected function _insertCollection($metadata = array())
    {
        ...
        if (($collection_id = $this->_harvest->collection_id)) {
            ...
            if (!isset($metadata['elementTexts']['Dublin Core']['Title'][0]['text']) || 
                ...
                $metadata['elementTexts']['Dublin Core']['Title'][0]['text'] = $this->_harvest->base_url;
            }
    ...
    final public function harvest()
    {
        try {
            ...
            $this->_harvest->save();
       /** This is the last thing it does before fail on insert collections**/ 
            $this->_beforeHarvest();
