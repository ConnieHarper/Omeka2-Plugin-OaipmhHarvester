...
class OaipmhHarvester_IndexController extends Omeka_Controller_AbstractActionController
{
   ...
   public function listsAction()
    {
        ...
        $baseUrl = trim($this->_getParam('base_url'));

        $waitTime = oaipmh_harvester_config('requestThrottleSecs', 5);
        if ($waitTime) {
            $request = new OaipmhHarvester_Request_Throttler(
                new OaipmhHarvester_Request($baseUrl),
                array('wait' => $waitTime)
            );
        } else {
            $request = new OaipmhHarvester_Request($baseUrl);
        }
        ...
        $availableMaps = array();
        if ($metadataFormats) {            
            foreach ($metadataFormats as $key=>$metadataFormat) {
                $metadataPrefix = (string) $key;	//->metadataPrefix;
 ...
 // Get the sets from the data provider.
        ...
        //$requestArguments['resumptionToken'] = $this->_getParam('resumption_token');

	$oaipmh = array();
        try {
        	$oaipmh = $request->listRecords($requestArguments);
        } catch (Zend_Uri_Exception $e) {
            $errorMsg = "Invalid URL given. $extraMsg";
        } catch (Zend_Http_Client_Exception $e) {
            $errorMsg = $e->getMessage() . " $extraMsg";
        ...
        if (isset($oaipmh['resumptionToken'])) {
            $resumptionToken = $oaipmh['resumptionToken'];
        } else {
       ...
       $goodRecords=array();
       foreach ($oaipmh['records'] as $record) {
    ...
    public function harvestAction()
    {
        ...
        if ($harvest_id) {
            ...
            $setName        = $this->_getParam('set_name');
            $setDescription = $this->_getParam('set_description');
	    $recordIdentifier = $this->_getParam('record_identifier');
            $search        =  $this->_getParam('search_text');
            $metadataPrefix = $this->_getParam('metadata_prefix');
            $harvest = $this->_helper->db->getTable('OaipmhHarvester_Harvest')->findUniqueHarvest($baseUrl, $setSpec, $metadataPrefix, $recordIdentifier);
